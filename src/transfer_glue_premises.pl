...
convert_fstr_to_premise(FStr, PremiseID, Premise) :-
    ...
    ( % This is the case where the meaning constructor is defined in terms
      ...
      % MR (this was REL in former versions of the implementation).
      % The meaning side will be the value of the MR attribute.
      member(['MR', RelVal], AttrVals), !,
      % Find the glue side. 
      ...
      % This is the case where there is no MR attribute.  We are
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% These functions are used for premises encoded with MR, TYPE,
% SEMSTR, and FORALL in the embedded format.

% Assemble the meaning expression. This will usually just be the value 
% of the MR attribute, but it could be a pointer to it. [New, 16 June 2020]
...
find_glue_args(BoundVars, FStr, [[Attr,_]|ArgAttrVals], Args):-
    % Ignore the MR, SEMSTR, TYPE, and FORALL attributes.  
    member(Attr, ['MR','SEMSTR','TYPE','FORALL']), !,
